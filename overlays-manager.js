class OverlayManager {
  ...
  renderSurahsList() {
    ...
    const createSurahItem = (surah, isPinned) => {
      ...
      <span>النزول:</span>
      <span class="order-value">${this.escapeHtml(String(surah.order))}</span></span>
      ...
      <span class="item-tertiary">(${this.escapeHtml(String(surah.verses))}) آية</span></span>
      </div>`;
  ...
  renderJuzHizbList() {
    ...
    hizbs.forEach((hizb) => {
      ...
      <span class="aya-prefix">${prefix}</span></span>
      </div>`;
