...
class QuranReader {
  ...
  updatePageInfo(page) {
    ...
    if (this.elements.pageNumber) this.elements.pageNumber.textContent = pageNum;

    const data = window.quranCalculator.getPageData(pageNum);
    if (!data) return;

    if (this.elements.hizbNumber) this.elements.hizbNumber.textContent = data.hizb || 1;
    ...
    if (notifications?.length) {
      const relevant = notifications.filter((n) =>
        n.includes("الجزء") || n.includes("الحزب") || n.includes("سجدة")
      );
  ...
  updateBookmarkButton() {
    ...
    const hasBookmark = window.quranApp.getBookmarks().some((b) => b.page === this.currentPage);
  ...
  showCurrentPageImage() {
    ...
    const pages = [this.currentPage - 1, this.currentPage, this.currentPage + 1]
      .filter((p) => p >= 1 && p <= 604);
  ...
  setupResizeObserver() {
    ...
    if (window.ResizeObserver) {
      ...
      if (this.elements.pageScroll) this.resizeObserver.observe(this.elements.pageScroll);
    } else {
  ...
  handleResize() {
    if (this.readingMode !== "scroll" || !this.spacer || this.resizePending) return;
    ...
    requestAnimationFrame(() => {
      ...
      this.wrapperPool.forEach((wrapper) => {
        ...
        if (!isNaN(p) && p > 0) wrapper.style.top = (p - 1) * this.pageHeight + "px";
      });
      this.elements.pageScroll.scrollTo({ top: scrollTarget, behavior: "auto" });
      this.updateVisiblePages();
      this.resizePending = false;
    });
  }
  ...
  updateVisiblePages() {
    if (this.readingMode !== "scroll") return;
    const container = this.elements.pageScroll;
    if (!container || !this.spacer || !this.pageHeight || this.pageHeight <= 0) return;
    ...
    if (toPage - fromPage + 1 < this.poolSize) {
      ...
      else if (toPage === 604) fromPage = Math.max(1, toPage - this.poolSize + 1);
    }
    ...
    this.wrapperPool.forEach((wrapper) => {
      ...
      if (top + this.pageHeight > viewTop && top < viewBottom) {
        if (img && img.dataset.loaded === "false") this.loadPageImage(page, "high");
      }
    });
  }
  ...
  detectCurrentPage() {
    if (!this.isRestoring && this.readingMode !== "book") this.detectCurrentPageScroll();
  }
  ...
  detectCurrentPageScroll() {
    ...
    const container = this.elements.pageScroll;
    if (!container || this.readingMode !== "scroll") return;
    const detected = Math.max(1, Math.min(604,
      Math.floor((container.scrollTop + container.clientHeight / 2) / this.pageHeight) + 1
    ));
  ...
  notifyPageChange() {
    ...
    window.dispatchEvent(new CustomEvent("quran:pageChanged", {
  ...
  setupSwipeNavigation() {
    ...
    const onTouchEnd   = (e) => {
      ...
      if (delta < -50 && this.currentPage < 604) this.goToPage(this.currentPage + 1);
      else if (delta > 50 && this.currentPage > 1) this.goToPage(this.currentPage - 1);
      startX = 0;
      endX   = 0;
    };
    ...
    this.elements.pageScroll.addEventListener("touchmove",  onTouchMove,  { passive: false });
    this.elements.pageScroll.addEventListener("touchend",   onTouchEnd);
    this.swipeHandlers = { touchstart: onTouchStart, touchmove: onTouchMove, touchend: onTouchEnd };
  }
  ...
  removeSwipeNavigation() {
    ...
    this.elements.pageScroll.removeEventListener("touchmove",  this.swipeHandlers.touchmove);
  ...
  setupTapToHide() {
    ...
    const onTouchEnd = (e) => {
      ...
      if (duration < 400 && deltaX < 20 && deltaY < 20 && now - this.lastTapTime > 300) {
    ...
    const onClick = (e) => {
      if (this.isTransitioning || !this.tapEnabled) return;
      const now = Date.now();
      if (now - this.lastTapTime < 300) { this.lastTapTime = now; return; }
      this.lastTapTime = now;
      if (e.target.closest(".header-btn") || e.target.closest(".footer-item") ||
          e.target.closest(".menu-overlay") || e.target.closest(".overlay")) return;
      this.toggleButtonsVisibility();
    };

    this.elements.pageScroll.addEventListener("touchstart", onTouchStart, { passive: false });
    this.elements.pageScroll.addEventListener("touchend",   onTouchEnd,   { passive: true });
    this.elements.pageScroll.addEventListener("click",      onClick);
    this.tapHandlers = { touchstart: onTouchStart, touchend: onTouchEnd, click: onClick };
  }
  ...
  toggleButtonsVisibility() {
    ...
    window.dispatchEvent(new CustomEvent("quran:buttonsVisibilityChanged", {
  ...
  enableTapDetection(enabled) { this.tapEnabled = enabled; }
  ...
  stopAutoScroll() {
    ...
    if (btnSlower) {
      ...
      setTimeout(() => { btnSlower.style.display = "none"; }, 200);
    }
    if (btnFaster) {
      btnFaster.classList.remove("visible");
      setTimeout(() => { btnFaster.style.display = "none"; }, 200);
    }
  }
  ...
  changeSpeed(direction) {
    let speed = Math.round((this.autoScrollSpeed + direction * this.speedStep) * 10) / 10;
  ...
  _updateSpeedButtons() {
    const btnSlower = document.getElementById("autoScrollSlower");
    const btnFaster = document.getElementById("autoScrollFaster");
    if (btnSlower) btnSlower.disabled = this.autoScrollSpeed <= this.minSpeed + 0.01;
    if (btnFaster) btnFaster.disabled = this.autoScrollSpeed >= this.maxSpeed - 0.01;
  }

  updateAutoScrollButton() {
    const el = document.getElementById("autoScrollGroup") || document.getElementById("autoScrollBtn");
    if (!el) return;
    if (this.readingMode === "scroll") {
      el.style.display = "";
    } else {
  ...
  setupEventListeners() {
    ...
    if (window.matchMedia) {
      ...
      const onOrientationChange = (e) => {
        if ((e.matches ? "book" : "scroll") !== this.readingMode) handleOrientationChange();
      };
      try {
        this.orientationMedia.addEventListener("change", onOrientationChange);
        this.eventListeners.push({ element: this.orientationMedia, type: "change", handler: onOrientationChange });
      } catch (err) {
        window.addEventListener("orientationchange", handleOrientationChange);
        this.eventListeners.push({ element: window, type: "orientationchange", handler: handleOrientationChange });
      }
    } else {
      window.addEventListener("orientationchange", handleOrientationChange);
      this.eventListeners.push({ element: window, type: "orientationchange", handler: handleOrientationChange });
    ...
    if (this.elements.bookmarkBtn) {
      ...
      this.eventListeners.push({ element: this.elements.bookmarkBtn, type: "click", handler });
    }
    ...
    if (this.elements.autoScrollBtn) {
      ...
      this.eventListeners.push({ element: this.elements.autoScrollBtn, type: "click", handler });
    }
    ...
    if (this.elements.pageNumber) {
      ...
      this.eventListeners.push({ element: this.elements.pageNumber, type: "click", handler });
    }
    ...
    this.eventListeners.push({ element: window, type: "quran:overlayOpened", handler: onOverlayOpened });
    ...
    this.eventListeners.push({ element: window, type: "quran:overlayClosed", handler: onOverlayClosed });
  }
  ...
  onAppResume() {
    this.isTransitioning = false;
    const overlayOpen = document.querySelector(".overlay.show, .menu-overlay.show") !== null;
    this.tapEnabled = !overlayOpen;
    this.applyReadingMode();
    setTimeout(() => {
      if (this.readingMode === "book") this.loadPageImage(this.currentPage, "high");
    }, 100);
  }
  ...
  destroy() {
    ...
    this.eventListeners.forEach(({ element, type, handler }) => {
      try { element.removeEventListener(type, handler); } catch (err) {}
    });
    this.eventListeners = [];
    this._cleanupTapHandlers();
    this.removeSwipeNavigation();
